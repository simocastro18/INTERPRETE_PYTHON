...
src/
├── main.cpp # Punto di ingresso principale
├── token/ # Sistema di tokenizzazione
│ ├── token.h
│ ├── token.cpp
│ └── tokenizer.h # Gestione indentazione e token speciali
├── parser/ # Parser per l'AST
│ ├── parser.h
│ └── parser.cpp
├── ast/ # Nodi dell'Abstract Syntax Tree
│ ├── nodes/ # Nodi per espressioni
│ │ ├── expressions/
│ │ └── literals/
│ └── statements/ # Nodi per statement
├── interpreter/ # Motore dell'interprete
│ ├── interpreter.h
│ └── interpreter.cpp
├── values/ # Sistema di gestione dei valori
│ ├── value.h
│ ├── value.cpp
│ └── types.h # Definizione tipi di dato
├── environment/ # Gestione degli scope
│ ├── environment.h
│ └── environment.cpp
